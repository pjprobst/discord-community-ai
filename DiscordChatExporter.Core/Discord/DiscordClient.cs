...
)
{
    ...
    )
    {
        ...
        foreach (var channel in channels)
        {
            ...
            {
                await foreach (
                    var thread in GetForumThreadsAsync(
                        channel,
                        includeArchived,
                        before,
                        after,
            ...
            {
                await foreach (
                    var thread in GetChannelThreadsAsync(
                        new[] { channel },
                        includeArchived,
                        before,
                        after,
    ...
    public async IAsyncEnumerable<Channel> GetForumThreadsAsync(
        Channel forum,
        bool includeArchived = false,
        Snowflake? before = null,
        Snowflake? after = null,
    ...
    {
        // Forum channels are just threads, so we can use the same logic as GetChannelThreadsAsync
        await foreach (
            var thread in GetChannelThreadsAsync(
                new[] { forum },
                includeArchived,
